...
# This script builds a Docker image and runs Docker container with the environment required to deploy and run Leancoin Token program.
...
start_docker_container_foreground_mode () {
  while true; do
    read -p "Do you want to automatically start test Solana validator and deploy Leancoin in the container? (y/n) " answer
    case $answer in 
      [yY]* ) docker run -p 8899:8899 --name $CONTAINER_NAME --rm -it $IMAGE_TAG bash -c "(solana-test-validator -q &) && sleep 10 && anchor deploy && echo 'Leancoin deployed and available on port: 8899.' && echo 'Once you exit this terminal, the container will be automatically stopped and removed. Use background mode to prevent this behaviour.' && bash"; exit;;
...
start_docker_container_background_mode () {
  ...
  while true; do
    read -p "Do you want to automatically start test Solana validator and deploy Leancoin in the container? (y/n) " answer
    case $answer in 
      [yY]* ) docker run -d -p 8899:8899 --name $CONTAINER_NAME $IMAGE_TAG bash -c "(solana-test-validator -q &) && sleep 10 && anchor deploy && echo 'Leancoin deployed and available on port: 8899.' && echo $BACKGROUND_CONTAINER_SUCCESS_MESSAGE && sleep infinity"; break;;
